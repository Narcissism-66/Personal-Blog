...
<template>
  <div class="grid grid-cols-[2fr,7fr] h-screen " >

<!--    推荐-->
    <div class="mt-24 gap-4 rounded-xl h-fit  w-4/5 ml-20 mt-22 mr-10 ">
      <div class="flex flex-wrap  gap-6">
        <span class="font-kai text-2xl text-blue-900 ml-16 w-full">热门话题</span>
        <div class="grid grid-cols-[1fr,7fr] gap-4">
...
<!--    Blogs栏-->
    <div class="flex flex-wrap  overflow-y-scroll mt-14 p-10 gap-8">
<!--      搜索框-->
      <div class="ml-64 flex flex-nowrap w-full">
        <input v-model="searchQuery.search" class="w-2/3 h-8 text-pink-400 font-kai text-xl border-blue-200 border-2 rounded-xl block focus:outline-none placeholder-pink-400 bg-transparent  items-center justify-center my-auto" placeholder="Your Title Here"/>
        <button @click="search">
...
<!--      gap只对flex/grid生效-->
      <div class="h-full flex flex-wrap gap-4">
        <div v-if="!IsSearch" class=" h-fit bg-opacity-5 shadow-xl select-none cursor-pointer hc-transition hover:scale-105 active:scale-100 rounded-xl p-1 mx-auto bg-gray-100" v-for="blog in options.blogs" @click="blogClickHandle(blog.blog.id)">
