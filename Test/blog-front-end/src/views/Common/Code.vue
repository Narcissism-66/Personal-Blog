...
try {
  monaco?.editor?.defineTheme?.('vs-dark-black', {
    ...
    colors: {
      ...
      'editor.selectionBackground': '#18212f',
      'editor.selectionHighlightBackground': '#1f2a3a',
      'editorIndentGuide.background': '#1f2937',
      'editorIndentGuide.activeBackground': '#374151',
      'editorWhitespace.foreground': '#2a3342',
      'scrollbarSlider.background': 'rgba(255,255,255,0.12)',
      'scrollbarSlider.hoverBackground': 'rgba(255,255,255,0.18)',
      'scrollbarSlider.activeBackground': 'rgba(255,255,255,0.24)',
      'editor.foreground': '#d4d4d4'
    }
  })
  monaco?.editor?.setTheme?.('vs-dark-black')
} catch (e) {
  // 忽略主题定义异常
}
...
const handleEditorMount = (editor, monaco) => {
  ...
  editor.onDidChangeModelContent(() => {
    if (currentFile.value) {
      ...
      // 仅更新长度统计，内容由 v-model:value 同步
      codeLength.value = content.length
    }
  })
  ...
  // 取消手动设置初始内容，改由 v-model:value 提供初始值并同步
}

const updateLanguage = () => {
  if (currentFile.value) {
    currentFile.value.language = currentLanguage.value
    // Monaco Editor 会通过 :language prop 自动更新语言
  }
}
...
const downloadFile = () => {
  if (!currentFile.value || !editorInstance.value) return
...
const selectFile = (file) => {
  ...
  // 由 v-model:value 负责把内容反映到编辑器；这里只统计长度
  codeLength.value = file.content.length
}
...
<template>
  <div class="h-screen flex flex-col bg-black text-white font-sans relative overflow-hidden">
    ...
    <!-- GitHub风格的顶部工具栏 -->
    <div class="flex justify-between items-center px-5 py-3 bg-black border-b border-gray-700 h-14 backdrop-blur-md relative z-10 shadow-lg">
    ...
    <div class="flex-1 flex overflow-hidden relative z-10 w-full min-w-0" @mousemove="handleResize" @mouseup="stopResize">
      <!-- 左侧文件区域 -->
      <div class="flex flex-col bg-black border-r border-gray-700 relative backdrop-blur-md min-w-0 flex-shrink-0 overflow-hidden transition-[width] duration-75 ease-out" :style="{ width: filePanelWidth + 'px' }">
        ...
        <!-- 文件区域调整手柄 -->
        <div
            class="absolute top-0 bottom-0 w-2 bg-transparent cursor-col-resize z-40 transition-all duration-200 flex items-center justify-center -right-1 hover:bg-white/5"
      ...
      <!-- 中间代码编辑区域 -->
      <div class="flex flex-col bg-black border-r border-gray-700 relative backdrop-blur-md min-w-0 flex-shrink-0 overflow-hidden transition-[width] duration-75 ease-out" :style="{ width: editorPanelWidth + 'px' }">
        <div class="flex justify-between items-center px-5 py-4 border-b border-gray-700 bg-gray-800 relative">
          <h3 class="m-0 text-sm font-bold text-white flex items-center gap-2">
            <div class="w-1 h-4 bg-gray-300 rounded-sm"></div>
            代码编辑器 (Monaco)
        ...
        <!-- Monaco Editor 容器 -->
        <div class="flex-1 overflow-hidden bg-black rounded-b-xl mx-2 mb-2 shadow-[inset_0_2px_8px_rgba(0,0,0,0.5)] w-full min-w-0">
          <MonacoEditor
              v-if="currentFile"
        ...
        <!-- 编辑器区域调整手柄 -->
        <div
            class="absolute top-0 bottom-0 w-2 bg-transparent cursor-col-resize z-10 md:z-30 transition-all duration-200 flex items-center justify-center -left-1 hover:bg-white/5"
            @mousedown="(event) => startResize('editor', event)"
        >
          <div class="w-0.5 h-10 bg-gray-700 rounded-sm transition-all duration-200 hover:h-15 hover:w-0.75 hover:bg-gray-300"></div>
        </div>
        <div
            class="absolute top-0 bottom-0 w-2 bg-transparent cursor-col-resize z-10 md:z-30 transition-all duration-200 flex items-center justify-center -right-1 hover:bg-white/5"
      ...
      <!-- 右侧输入输出区域 -->
      <div class="flex flex-col bg-black border-r border-gray-700 relative backdrop-blur-md min-w-0 flex-shrink-0 overflow-hidden transition-[width] duration-75 ease-out" :style="{ width: outputPanelWidth + 'px' }">
        <div class="flex justify-between items-center px-5 py-4 border-b border-gray-700 bg-gray-800 relative">
          <h3 class="m-0 text-sm font-bold text-white flex items-center gap-2">
            <div class="w-1 h-4 bg-gray-300 rounded-sm"></div>
            输入输出
    ...
    <!-- GitHub风格的底部状态栏 -->
    <div class="flex justify-between items-center px-5 py-2 bg-black border-t border-gray-700 h-10 text-xs font-medium backdrop-blur-md relative z-10 shadow-lg">
...
::deep(.monaco-editor .minimap-decorations-layer),
::deep(.monaco-editor .minimap-shadow-hidden) {
