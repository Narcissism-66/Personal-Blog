...
public interface BlogMapper {
    ...
    //查询所有博客，按id序
    @Select("SELECT id,title,authorId,substring(content,1,120) AS content FROM blog ORDER BY id DESC LIMIT 50")
    ...
    //搜索Blog
    @Select("SELECT id,title,authorId,substring(content,1,200) AS content FROM blog WHERE title LIKE CONCAT('%',#{title},'%') ORDER BY id DESC LIMIT 50 ")
